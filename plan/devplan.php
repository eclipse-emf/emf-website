...
function getSQLData($query) { 
	...
	if(mysql_errno($dbh) > 0) {
		return array("ERROR" => "There was an error processing the request:\n\n$query");
...
function getCommitterList() { // not available in bugzilla, this is a CVS property
...
function getMetaAndBugListSQL() {
	...
	$query = "
SELECT DISTINCT 
  BUG.bug_id, BUG.bug_severity, BUG.priority, BUG.rep_platform, 
  BUG.assigned_to, BUG.bug_status, BUG.resolution, BUG.short_desc, 
...
WHERE 
  ...
  (PROD.name = 'EMF' OR PROD.name = 'XSD') AND
    ( BUG.bug_status = 'NEW' OR
      BUG.bug_status = 'ASSIGNED' OR
      BUG.bug_status = 'REOPENED' 
    )
ORDER BY
  bug_id
DESC
";
	$data = getSQLData($query);
	$num=0;
	foreach ($data as $i => $pairs) { 
		foreach ($pairs as $k => $v) {
			if ($k=="bug_id") { // collect $buglist
				if ($buglist) { $buglist.=","; }
				$buglist.=$v;
				$num=$v;
			} else if ($num) { // collect fields in $bugz
				if (!is_array($bugz[$num])) { $bugz[$num] = array(); }
				$bugz[$num][$k] = $v;
			}
		}
	}
