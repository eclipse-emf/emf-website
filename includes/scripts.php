<?php 
	...
	// $Id: scripts.php,v 1.1 2004/12/07 22:03:03 nickb Exp $ 
	...
	function getPWD($suf="") {
		...
		if (!is_dir($PWD) || !is_readable($PWD) || ($suf=="logs" && !is_writable($PWD)) ) { 
			if ($_SERVER["HTTP_HOST"]=="emf.torolab.ibm.com" || $_SERVER["HTTP_HOST"]=="emf") {
				if ($suf=="logs") { 
					$PWD = "/home/www-data/".$suf; // non-standard location (not in web path)
				} else if ($suf=="downloads/drops") {
					$PWD = "/home/www-data/emf-build/tools/emf/downloads/drops"; 
				}
			} else if ($_SERVER["HTTP_HOST"]=="download1.eclipse.org") {
				$PWD = "/home/www/eclipse/tools/emf/".$suf;
				//$PWD = "/home/www/tools/emf/downloads/drops"; 
		...
		if (!is_dir($PWD) || !is_readable($PWD) || ($suf=="logs" && !is_writable($PWD)) ) { 
			...
			if (is_dir($PWD) && is_readable($PWD)) { 
				// try 1:
				$PWD = "/home/www/tools/emf/".$suf; // default path
	...
	function getNews($lim,$key) {
		...
		global $CVSpre,$pre; // $CVSpreDocEMF,
		$xml = file($CVSpre."news/news.xml"); // moved from .doc plugin to *-home package to simplify the build
		if (!$xml) {
			$xml = array();
		}
		$xmlCollect=0;
			...
			if ($lim<0 || $i<$lim) {
				...
				foreach ($pair as $date => $contents) { 
					...
					if (strtotime($date)>strtotime("-3 weeks")) { 
						echo '<img src="'.$CVSpre.'images/new.gif" width="31" height="14">';
					}
